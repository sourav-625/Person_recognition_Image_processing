...
#
def predict_people_in_image(model: keras.Model, class_names: List[str], image_path: str, simulate_count: int = 3) -> List[str]:
    """
    Given a single image file path, simulate detecting multiple faces by creating a small batch
    (simulate_count) of images derived from the original image (resized/perturbed copies),
    then preprocess them with the same FaceNet preprocessing pipeline before prediction.

    Returns: list of identified class names (length == simulate_count)
    """
    ...
    for i in range(simulate_count):
        # Create multiple simulated crops (resize + small deterministic perturbations)
        # crop_w = int(w * (0.6 + 0.1 * (i % 2)))   # vary crop size
        # crop_h = int(h * (0.6 + 0.1 * ((i + 1) % 2)))
        # left = max(0, int((w - crop_w) * (i / max(1, simulate_count - 1))))
        # upper = max(0, int((h - crop_h) * ((simulate_count - 1 - i) / max(1, simulate_count - 1))))
        # box = (left, upper, left + crop_w, upper + crop_h)

        crop = pil

        # angle = (-5) + (i * 5)  # e.g., -5, 0, 5 degrees
        # crop = crop.rotate(angle, resample=Image.BILINEAR, expand=False)

        crop_arr = tf.convert_to_tensor(np.array(crop), dtype=tf.float32)
        crop_arr = preprocess_image(crop_arr)  # ensures 160x160 resize + normalization
        crop_arr = crop_arr.numpy()  # back to numpy for batching
    ...
    print("\n[predict_people_in_image] Simulated multi-person identification results:")
    for i, name in enumerate(identified, start=1):
        print(f"  - Simulated detected face #{i}: {name}")
    print("")
    return identified
...
def main():
    ...
    print(f"[main] Running simulated multi-person prediction on sample image: {sample_path}")
    identified = predict_people_in_image(model, class_names, 'niranjan_test.jpg', simulate_count=1)
    identified = predict_people_in_image(model, class_names, 'sourav_test.jpg', simulate_count=1)

    print("[main] Demo complete. Identified:", identified)
    print("[main] Note: The multi-person identification above is simulated using multiple crops of the same image.\n")
